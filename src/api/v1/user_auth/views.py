...
from src.api.v1.user_auth.schemas import (
    AuthorizeUserInSchema,
    AuthorizeUserOutSchema,
    LoginUserInSchema,
    LoginUserOutSchema,
)
...
async def authorize_user_views(
    ...
    command = AuthorizeUserCommand(authorize_in.to_entity())
...
async def login_user_views(
    login_in: LoginUserInSchema, container: punq.Container = Depends(get_container)
    ...
    return ApiResponse(data=LoginUserOutSchema(token=token))
