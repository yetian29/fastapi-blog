...
@pytest.fixture
def mock_test_authorize_user_auth_use_case(mock_test_container):
    return mock_test_container.resolve(AuthorizeUseAuthUseCase)


@pytest.fixture
def mock_test_login_user_auth_use_case(mock_test_container):
...
async def test_login_user_auth_use_case(
    ...
    code = await mock_test_authorize_user_auth_use_case.execute(command1)
    command2 = LoginUserAuthCommandFactory.build(
        phone_number=command1.user.phone_number, email=command1.user.email, code=code
    )
    token = await mock_test_login_user_auth_use_case.execute(command2)
